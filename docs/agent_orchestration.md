...
2. **`qa-tester`**: The QA Engineer. Writes the tests in `tests/` based on the specification, without touching the code.
...
2. **Testing**: Once the specification is created, the user calls the `qa-tester` to generate the test suite in `tests/`.
